...
#ifdef __SPECIAL_PVP_DUEL_OPTION__
#include "pvp.h"
#endif
...
LPITEM CHARACTER::GetInventoryItem(const WORD& wCell) const
{
...
#endif
{
	...
	if (GetDesc())
	{
		...
		{
#ifdef __GROWTH_PET_SYSTEM__
			if (pItem->IsPetUpbringing() && GetGrowthPetSystem())
			{
				GetGrowthPetSystem()->UpdateItem(pItem->GetID());
			}
#endif
...
bool CHARACTER::IsEmptyItemGrid(const TItemPos& Cell, BYTE bSize, int iExceptionCell) const
{
...
bool CHARACTER::CheckTimeUsed(LPITEM item)
{
	...
	case 950821:	case 950822:	case 950823:	case 950824:	case 950825:	case 950826:

		///SubType = 7 -> USE_ABILITY_UP
	...
	case 76003:	case 76017:	case 76018: case 50815: case 50816:
		...
		if (pGetFlag)
		{
			...
			{
				if (test_server)
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Wait %u seconds to use this again."), pGetTime[0]);
...
bool CHARACTER::UseItemEx(LPITEM item, TItemPos DestCell)
{
...
#ifdef __SPECIAL_PVP_DUEL_OPTION__
		if (CPVPManager::instance().InSpecialDuel(this))
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You cannot use this item in a special duel."));
			return false;
		}
#endif
	...
	case ITEM_USE:
	{
		...
		case USE_TIME_CHARGE_FIX:
		{
			...
			if (pDestItem->IsDragonSoul())
			{
#ifdef __EXTENDED_DSS_RECHARGE__
			...
			else if (pDestItem->IsCostume() && !pDestItem->IsCostumeAcce())
			{
				...
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s is now with infinite time."), pDestItem->GetName());
					item->SetCount(item->GetCount() - 1);
					return true;
				}
				else
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s already has infinite time."), pDestItem->GetName());
					return false;
				}
			}
#endif
		...
		case USE_SPECIAL:
			...
			case 38060:
			{
				...
				std::mt19937 gen(rd());
				std::uniform_real_distribution<> dis(61, 78);
				int iSpecialActionID = dis(gen);
				long lExpireTime = 60 * 60 * 12; // 12h
				long lDuration = get_global_time() + lExpireTime;

				char szQuestFlag[20] = "0";
				snprintf(szQuestFlag, sizeof(szQuestFlag), "emotions.expire_%d", iSpecialActionID);
				SetQuestFlag(szQuestFlag, lDuration);

				ChatPacket(CHAT_TYPE_COMMAND, "RegisterSpecialEmotion %d %d", iSpecialActionID, lDuration);
				item->SetCount(item->GetCount() - 1);
			}
			break;
#endif
			...
			case 93100:
			{
				...
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You have already completed this battle pass."));
					return false;
				}

				if (FindAffect(AFFECT_BATTLE_PASS))
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You already have a battle pass activated on this character."));
					return false;
				}
				else
				{
					AddAffect(AFFECT_BATTLE_PASS, POINT_BATTLE_PASS_ID, item->GetValue(0), 0, item->GetValue(1), 0, true);
					ITEM_MANAGER::instance().RemoveItem(item);
				}
			}
			break;
...
#ifdef __ACCE_COSTUME_SYSTEM__
			if (item->GetValue(0) == ACCE_CLEAN_ATTR_VALUE0 && item->IsAcceReverseScroll())
			{
...
#ifdef __CHANGE_LOOK_SYSTEM__
			if (item->GetValue(0) == CL_CLEAN_ATTR_VALUE0 && item->IsChangeLookReverseScroll())
			{
...
#endif
		{
			...
			*/
			...
			if ((item2->IsCostumeHair() || item2->IsCostumeWeapon() || item2->IsCostumeBody())
				&& !item->IsCostumeModifyItem() && !(item2->IsCostumeMount() && item2->IsCostumeAcce()))
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You cannot change the upgrade of this item."));
				return false;
			}
			...
			case USE_ADD_ATTRIBUTE:
				...
				{
#ifdef __USE_SINGLE_ADD_ATTRIBUTE__
					// 축복의 구슬 
	...
	case ITEM_PET:
	{
		...
		{
			if (GetGrowthPetSystem() && GetGrowthPetSystem()->IsActivePet())
			{
				if (GetGrowthPetSystem()->DeleteAllSkills())
					ITEM_MANAGER::instance().RemoveItem(item);
			}
			else
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Activate your pet first."));
			}
		}
		break;
		default:
...
#ifdef __GROWTH_PET_SYSTEM__
	if (GetGrowthPetSystem()->IsActivePet() && item->IsPetUpbringing())
	{
...
#ifdef __EXTENDED_ITEM_COUNT__
bool CHARACTER::MoveItem(const TItemPos& Cell, TItemPos DestCell, short count)
#else
bool CHARACTER::MoveItem(const TItemPos& Cell, TItemPos DestCell, BYTE count)
#endif
{
	LPITEM item = NULL;
		...
		if (item->IsOwnership(this))
		{
			...
			else
			{
				...
				else if (item->IsSkillBook())
				{
					...
					if (iOpenCount < 3)
					{
						ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("%s was added to your special inventory."), item->GetName());
...
#ifdef __MOUNT_COSTUME_SYSTEM__
bool CHARACTER::UnequipItem(LPITEM item, bool unequipFromPlayer /*= false*/)
#else
...
bool CHARACTER::EquipItem(LPITEM item, int iCandidateCell)
{
	...
	if (true == item->IsEquipped())
	{
		...
		/* // Gem Pendant (#) UNUSED
		else if (true == CItemVnumHelper::IsGemPendant(dwVnum))
		{
...
#ifdef __WJ_PICKUP_ITEM_EFFECT__
#ifdef __EXTENDED_ITEM_COUNT__
LPITEM CHARACTER::AutoGiveItem(DWORD dwItemVnum, short bCount, int iRarePct, bool bMsg, bool isHighLight)
#else
LPITEM CHARACTER::AutoGiveItem(DWORD dwItemVnum, BYTE bCount, int iRarePct, bool bMsg, bool isHighLight)
#endif
#else
#ifdef __EXTENDED_ITEM_COUNT__
...
#ifdef __MOUNT_COSTUME_SYSTEM__
bool CHARACTER::UnEquipSpecialRideUniqueItem(bool isOnDeath)
#else
...
#ifdef __MOUNT_COSTUME_SYSTEM__
	if (item->IsCostumeMount() && GetMapIndex() == 113)
	{
...
bool CHARACTER::CanUnequipNow(const LPITEM item, const TItemPos& swapCell) /*const*/
{
	...
	{ // 09.05.2019.owsap Remove SKILL_GWIGEOM & SKILL_GEOMKYUNG if not using weapon
		if (item->GetType() == ITEM_WEAPON)
		{
			if (IsAffectFlag(AFF_GWIGUM))
				RemoveAffect(SKILL_GWIGEOM);
	...
	// 아이템 unequip시 인벤토리로 옮길 때 빈 자리가 있는 지 확인
	{
		...
		// Not enough space, but we are swapping, so let's figure out if I can unequip after the theorical swap
		if (pos == -1 && !item->IsDragonSoul())
		{
			if (swapCell != NPOS)
			{
